...
export const UPLOAD_LINK_DESCRIPTION =
  "Uploaded-file links support `expires_in`, `one_time_use`, and `session_duration`; `access_policy` and `max_sessions` are rejected. **Revocation:** a link created here cannot be revoked from this server; `delete_qurl` on the returned `resource_id` does not stop it (revocation needs the connector's `/api/revoke_links`). Use a short `expires_in` and `one_time_use` for sensitive files. `label` names the email and PDF only; it is not attached to the link, so `list_qurls` cannot find the link by it. ";
