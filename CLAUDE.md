...
- **Customer expiry invariant:** [qurl-service #1511](https://github.com/layervai/qurl-service/pull/1511) defines a 30-day link-lifetime maximum for every customer plan. Preserve the create/mint and quota descriptions when refreshing this older snapshot; `src/__tests__/api-spec-expiry.test.ts` guards against restoring the former 3-day Free limit. The canonical source is [qurl-service/api/openapi.yaml](https://github.com/layervai/qurl-service/blob/main/api/openapi.yaml). A drift report against an older deployed spec is not a reason to revert this policy. The public HTTP duration ceiling is 30 days even for internal System accounts; their reported internal quota can differ. Deliberate future policy changes must update the guard with the source contract.
